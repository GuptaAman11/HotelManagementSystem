...
public class UpdateCheck extends JFrame {
	...
	public UpdateCheck() throws SQLException {
		...
		JLabel lblUpdateCheckStatus = new JLabel("Update customer");
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) { 
				...
				String room_no = txt_ID.getText(); //room_number;    
                                String name_cus = txt_Status.getText(); //name    
                                String s4 = txt_Date.getText(); //status;    
                                String s5 = txt_Time.getText(); //deposit    
				
                                c.s.executeUpdate("update customer set allo_room = '"+room_no+"', name = '"+name_cus+"', check1 = '"+s4+"', deposit = '"+s5+"' where number = '"+s1+"'");
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String number = c1.getSelectedItem();
                                String querry = "select * from customer where number = '"+number+"'" ;
				conn c = new conn();
                                ResultSet rs1 = c.s.executeQuery(querry);
                                
                                while(rs1.next()){
                                    txt_ID.setText(rs1.getString("allo_room"));    
                                    txt_Status.setText(rs1.getString("name"));    
                                    txt_Date.setText(rs1.getString("check1"));    
